...
describe("session id computation", function()
    ...
    it("is the first six hex chars of the digest", function()
        ...
        assert.equal("00ac56", session.session_id(instance_id, "/srv/code/proj"))
    end)

    it("is six characters long", function()
        assert.equal(6, #session.session_id(instance_id, "~/code/proj"))
    end)

    it("is lowercase hex", function()
        local id = session.session_id(instance_id, "~/code/proj")

        assert.truthy(id:find "^[0-9a-f]+$", id)
    end)

    it("distinguishes paths - a different project is a different session", function()
        local one = session.session_id(instance_id, "/srv/code/proj")
        local other = session.session_id(instance_id, "/srv/code/other")
    ...
    it("distinguishes outposts - a different instance id is a different session", function()
        local one = session.session_id(instance_id, "/srv/code/proj")
        local other = session.session_id("00000000-0000-0000-0000-000000000000", "/srv/code/proj")
    ...
    it("joins instance id and path with a separator, not bare concatenation", function()
        ...
        assert.equal("00ac56", session.session_id(instance_id, "/srv/code/proj"))
        assert.not_equal("b488dd", session.session_id(instance_id, "/srv/code/proj"))
    end)
end)
